...
while(True):
    try:
        ...
        with sr.Microphone() as source2:
            ...
            MyText_doc = nlp(MyText)
            #MyText_sents = ' '.join([sent.text for sent in MyText_doc.sents if len(sent.text) > 1])
            for sentence in MyText_doc.sents:
                print(sentence.text)
                write_to_file(sentence.text)
            ...
            # Markov Model
            new_sentence = generate_sentence("output.txt", 1, 50)
